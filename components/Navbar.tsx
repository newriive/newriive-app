...
export default function Navbar() {
  ...
  useEffect(() => {
    getCurrentUserSession()
      .then((session) =>
        setIsLoggedIn(!!session && session.isValid && session.isValid()),
      )
  ...
  const mainNavItems = filteredNavItems.filter(
    (item) => !['/login', '/signup'].includes(item.href),
  )
  const authNavItems = filteredNavItems.filter((item) =>
    ['/login', '/signup'].includes(item.href),
  )
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-lg p-8 min-w-[320px] max-w-sm relative">
            <button
              ...
              className="absolute top-2 right-2 text-brand-indigo hover:text-brand-gold text-xl"
            >
              ×
            </button>
            <LoginPage />
